...
/* 
  射线方程, P(t)=o+t⋅d
  * o: 射线起点, 类型 vec3
  * d: 射线方向, 通常是一个单位向量, 类型为 vec3
  * t: 参数化值, 表示射线起点延射线方向的距离. 如果 t > 0, 表示射线的前进方向; t < 0表示在反方向
*/
/* 
  球体方程: (x - c.x)² + (y - c.y)² + (z - c.z)² = r² 
  * (c.x, c.y, c.z): 球体中心的坐标
  * r: 球体的半径
*/
/* 
  ...
  * c: 圆的中心点
  ...
  展开上个公式: (o - c + t·d) · (o - c + t·d) = r²
*/
/* 
  最终得二次方程: (o - c) · (o - c) + 2t·d·(o - c) + t²·(d·d) = r²
  进一步整理得标准的二次方程形式: at² + bt + c = 0 其中:
  * a = dot(d, d): 射线方向向量的自乘点积. 由于 d 通常是一个单位向量, 所以 a = 1, 但在一般情况下它是射线方向向量的平房长度.
  * b = 2.0 * dot(d, oc): o - c是射线原点到球心的向量,
  * c = dot(oc, oc) - (radius * radius): 射线起点到球心的向量长度的平方, 再减去球体半径的平方.
*/
/* 
  二次方程: at² + bt + c = 0, 为了判断这个二次方程是否有实数解, 需要使用判别式.
  二次方程的判别式: △ = b² - 4ac.
  * 如果 △ > 0: 二次方程有两个不同的实数根, 表示射线与球体有两个交点. 这意味着射线穿过了球体.
  * 如果 △ = 0: 二次方程有一个实数根, 表示射线与球体有一个交点. 这意味着射线与球体相切.
  * 如果 △ < 0: 二次方程没有实数根, 表示射线没有与球体相交.
*/

/* 
  二次方程求根公式: t = (-b ± sqrt(b² - 4ac)) / 2a
  * det = (b * b) - (4.0 * a * c);
  * t0 = (-b - sqrt(det)) / (2.0 * a);
  * t1 = (-b + sqrt(det)) / (2.0 * a);
*/

czm_raySegment czm_raySphereIntersectionInterval(
  czm_ray ray,
  vec3 center,
  float radius
){
...
float czm_private_shadowVisibility(
  float visibility,
  float nDotL,
  float normalShadingSmooth,
  float darkness
...
float czm_shadowVisibility(
  ...
  zm_shadowDepthCompare(shadowMap,uv+vec2(dx0,dy0),depth)+
...
void czm_writeLogDepth(float depth){
  ...
  if(depth<=.9999999f||depth>czm_farDepthFromNearPlusOne){
    discard;
  }
  #ifdef POLYGON_OFFSET
...
void main(){
  ...
  float fade=max((depth-shadowMapMaximumDistance*.8f)/
  (shadowMapMaximumDistance*.2f),0.0);
