...
vec4 czm_updatePositionDepth(vec4 coords) {
  ...
  #if defined(LOG_DEPTH)#ifdef SHADOW_MAP    
...
void czm_vertexLogDepth() {
  ...
  #ifdef LOG_DEPTH    
  v_depthFromNearPlusOne = (gl_Position.w - czm_currentFrustum.x) + 1.0f;
...
void czm_vertexLogDepth(vec4 clipCoords) {
  ...
  #ifdef LOG_DEPTH    
  v_depthFromNearPlusOne = (clipCoords.w - czm_currentFrustum.x) + 1.0f;
...
vec4 geometryStage(inout ProcessedAttributes attributes, mat4 modelView, mat3 normal) {
  ...
  defined(ENABLE_CLIPPING_POLYGONS) v_positionWC = (czm_model * vec4(positionMC, 1.0f)).xyz;    
  #endif                                                
...
void czm_log_depth_main() {
  ...
  #endif    #ifdef HAS_CUSTOM_VERTEX_SHADER    
