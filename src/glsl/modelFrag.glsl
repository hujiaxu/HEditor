...
czm_raySegment czm_raySphereIntersectionInterval(czm_ray ray, vec3 center, float radius){    
  vec3 o = ray.origin;    vec3 d = ray.direction;    vec3 oc = o - center;    float a = dot(d, d);    float b = 2.0 * dot(d, oc);    float c = dot(oc, oc) - (radius * radius);    
  float det = (b * b) - (4.0 * a * c);    
  if (det < 0.0) {        return czm_emptyRaySegment;    }    
  float sqrtDet = sqrt(det);    float t0 = (-b - sqrtDet) / (2.0 * a);    float t1 = (-b + sqrtDet) / (2.0 * a);    czm_raySegment result = czm_raySegment(t0, t1);    
  return result;
}

float czm_sampleShadowMap(highp samplerCube shadowMap, vec3 d){    return czm_unpackDepth(czm_textureCube(shadowMap, d));}

float czm_sampleShadowMap(highp sampler2D shadowMap, vec2 uv){
  #ifdef USE_SHADOW_DEPTH_TEXTURE    return texture(shadowMap, uv).r;
  #else    return czm_unpackDepth(texture(shadowMap, uv));
  #endif
}
float czm_shadowDepthCompare(samplerCube shadowMap, vec3 uv, float depth){    
  return step(depth, czm_sampleShadowMap(shadowMap, uv));
}
float czm_shadowDepthCompare(sampler2D shadowMap, vec2 uv, float depth){    return step(depth, czm_sampleShadowMap(shadowMap, uv));}

struct czm_shadowParameters{
  #ifdef USE_CUBE_MAP_SHADOW    vec3 texCoords;
  #else    vec2 texCoords;#endif    float depthBias;    float depth;    float nDotL;    vec2 texelStepSize;    float normalShadingSmooth;    float darkness;
};
...
vec3 czm_HSBToRGB(vec3 hsb){    vec3 p = abs(fract(hsb.xxx + K_HSB2RGB.xyz) * 6.0 - K_HSB2RGB.www);    return hsb.z * mix(K_HSB2RGB.xxx, clamp(p - K_HSB2RGB.xxx, 0.0, 1.0), hsb.y);}
const vec4 K_RGB2HSB = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
vec3 czm_RGBToHSB(vec3 rgb){    
  vec4 p = mix(vec4(rgb.bg, K_RGB2HSB.wz), vec4(rgb.gb, K_RGB2HSB.xy), step(rgb.b, rgb.g));    vec4 q = mix(vec4(p.xyw, rgb.r), vec4(rgb.r, p.yzx), step(p.x, rgb.r));    
...
void czm_computeScattering(    
  ...
  for (int i = 0; i < PRIMARY_STEPS_MAX; ++i) {                        
    ...
    vec3 attenuation = exp(
      -((czm_atmosphereMieCoefficient * (opticalDepth.y + lightOpticalDepth.y)) + (czm_atmosphereRayleighCoefficient * (opticalDepth.x + lightOpticalDepth.x)))
    );                            
...
float czm_branchFreeTernary(bool comparison, float a, float b) {    float useA = float(comparison);    return a * useA + b * (1.0 - useA);}
vec2 czm_branchFreeTernary(bool comparison, vec2 a, vec2 b) {    float useA = float(comparison);    return a * useA + b * (1.0 - useA);}
vec3 czm_branchFreeTernary(bool comparison, vec3 a, vec3 b) {    float useA = float(comparison);    return a * useA + b * (1.0 - useA);}
vec4 czm_branchFreeTernary(bool comparison, vec4 a, vec4 b) {    float useA = float(comparison);    return a * useA + b * (1.0 - useA);}
float czm_private_shadowVisibility(float visibility, float nDotL, float normalShadingSmooth, float darkness){
  #ifdef USE_NORMAL_SHADING
  #ifdef USE_NORMAL_SHADING_SMOOTH    
  float strength = clamp(nDotL / normalShadingSmooth, 0.0, 1.0);#else    float strength = step(0.0, nDotL);
...
float czm_shadowVisibility(sampler2D shadowMap, czm_shadowParameters shadowParameters){    
  ...
  depth -= depthBias;#ifdef USE_SOFT_SHADOWS    
...
void czm_writeLogDepth(float depth){
  ...
  #ifdef POLYGON_OFFSET        float factor = u_polygonOffset[0];    float units = u_polygonOffset[1];
...
vec3 lambertianDiffuse(vec3 diffuseColor){    return diffuseColor / czm_pi;}
...
vec4 czm_computeAtmosphereColor(    czm_ray primaryRay,    vec3 lightDirection,    vec3 rayleighColor,    vec3 mieColor,    float opacity) {    
...
vec3 czm_getDynamicAtmosphereLightDirection(vec3 positionWC, float lightEnum) {    
  ...
  const float SCENE_LIGHT = 1.0;    const float SUNLIGHT = 2.0;    vec3 lightDirection =        
...
vec3 czm_sphericalHarmonics(vec3 normal, vec3 coefficients[9]){    
  ...
  vec3 L10 = coefficients[2];    vec3 L11 = coefficients[3];    
  vec3 L2_2 = coefficients[4];    vec3 L2_1 = coefficients[5];    
  vec3 L20 = coefficients[6];    vec3 L21 = coefficients[7];    
  vec3 L22 = coefficients[8];    float x = normal.x;    
  float y = normal.y;    float z = normal.z;    
  vec3 L =          
      L00        
    + L1_1 * y        
    + L10 * z        
...
struct SelectedFeature{    float _empty;};struct FeatureIds{    float _empty;};struct Metadata{    float _empty;};
...
vec2 nearestPointOnEllipseFast(vec2 pos, vec2 radii) {    
  vec2 p = abs(pos);    vec2 inverseRadii = 1.0 / radii;    
...
void applyFog(inout vec4 color, vec4 groundAtmosphereColor, vec3 lightDirection, float distanceToCamera) {    
  ...
  fogColor.rgb = czm_inverseGamma(fogColor.rgb);    #endif    
...
void atmosphereStage(inout vec4 color, in ProcessedAttributes attributes) {    
  vec3 rayleighColor;    vec3 mieColor;    
  float opacity;    vec3 positionWC;    
  vec3 lightDirection;                    
  if (false) {        
    ...
    positionWC = attributes.positionWC;        lightDirection = czm_getDynamicAtmosphereLightDirection(positionWC, czm_atmosphereDynamicLighting);        
...
NormalInfo getNormalInfo(ProcessedAttributes attributes){    
  ...
  vec3 bitangent = normalize(cross(geometryNormal, tangent));    #endif    
  #ifdef HAS_NORMAL_TEXTURE        
...
vec3 getNormalFromTexture(ProcessedAttributes attributes, vec3 geometryNormal){    
  ...
  vec3 normalSample = texture(u_normalTexture, normalTexCoords).rgb;    normalSample = 2.0 * normalSample - 1.0;    
  #ifdef HAS_NORMAL_TEXTURE_SCALE        
  normalSample.xy *= u_normalTextureScale;    
  #endif    
...
vec3 computeNormal(ProcessedAttributes attributes){        
  ...
  #if defined(HAS_NORMAL_TEXTURE) && !defined(HAS_WIREFRAME)        
  normal = getNormalFromTexture(attributes, normal);    #endif    
  #ifdef HAS_DOUBLE_SIDED_MATERIAL        
  if (czm_backFacing()) {            
    normal = -normal;        
  }    
...
void setSpecularGlossiness(inout czm_modelMaterial material){    
  ...
  #endif    material.diffuse = diffuse.rgb * (1.0 - czm_maximumComponent(specular));           
...
void setSpecular(inout czm_modelMaterial material, in float metalness){    
  ...
  vec3 f0 = material.specular;    vec3 dielectricSpecularF0 = min(f0 * specularColorFactor, vec3(1.0));    
...
void setAnisotropy(inout czm_modelMaterial material, in NormalInfo normalInfo){    
  ...
  material.anisotropicT = anisotropicT;    material.anisotropicB = anisotropicB;    
...
void setClearcoat(inout czm_modelMaterial material, in ProcessedAttributes attributes){    
  ...
  #endif        float clearcoatRoughness = texture(u_clearcoatRoughnessTexture, clearcoatRoughnessTexCoords).g;        
  ...
  #endif    
  #endif    material.clearcoatFactor = clearcoatFactor;        
...
void czm_log_depth_main(){    
  ...
  if (v_pointCloudShow == 0.0)        {            discard;        }    
  #endif    
...
#endif vec4 getPositionEC() {     return vec4(v_positionEC, 1.0); } 
vec3 getNormalEC() {     return vec3(1.0); } 
void applyNormalOffset(inout vec4 positionEC, vec3 normalEC, float nDotL) { } 
void main() {     
  czm_shadow_receive_main();     
  vec4 positionEC = getPositionEC();     vec3 normalEC = getNormalEC();     
  ...
  shadowParameters.texCoords = shadowPosition.xy;     shadowParameters.depth = shadowPosition.z;     
