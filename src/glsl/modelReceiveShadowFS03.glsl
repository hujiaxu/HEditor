...
float czm_shadowVisibility(sampler2D shadowMap, czm_shadowParameters shadowParameters) {
  ...
  float visibility = (
    ...
    + czm_shadowDepthCompare(shadowMap, uv 
    + vec2(0.0, dy0), depth) 
...
mat4 czm_cascadeMatrix(vec4 weights) {
  return shadowMap_cascadeMatrices[0] 
    * weights.x + shadowMap_cascadeMatrices[1] 
    * weights.y + shadowMap_cascadeMatrices[2] 
    * weights.z + shadowMap_cascadeMatrices[3] 
    * weights.w;
}
...
void czm_writeLogDepth(float depth) {
  ...
  if(depth <= 0.9999999f || depth > czm_farDepthFromNearPlusOne) {
    discard;
  }
  #ifdef POLYGON_OFFSET        
...
vec4 czm_computeAtmosphereColor(
  ...
  float miePhase = 3.0 / (25.1327412287f) * ((1.0 - GSq) * (cosAngleSq + 1.0)) / (pow(1.0 + GSq - 2.0 * cosAngle * G, 1.5) * (2.0 + GSq));
...
void atmosphereStage(inout vec4 color, in ProcessedAttributes attributes) {
    vec3 rayleighColor;    
    vec3 mieColor;    
    float opacity;    vec3 positionWC;    vec3 lightDirection;                    
    if (false) {        
...
void czm_shadow_receive_main() {     czm_log_depth_main();     czm_writeLogDepth(); }
