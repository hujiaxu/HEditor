...
struct czm_shadowParameters {
  ...
  #else    vec2 texCoords;
  #endif    float depthBias;    float depth;    float nDotL;    vec2 texelStepSize;    float normalShadingSmooth;    float darkness;
};
...
const float czm_pi = 3.141592653589793f;
...
void czm_computeScattering(
  ...
  const float ATMOSPHERE_THICKNESS = 111e3f;
...
float czm_shadowVisibility(sampler2D shadowMap, czm_shadowParameters shadowParameters) {
  ...
  depth -= depthBias;
  #ifdef USE_SOFT_SHADOWS    vec2 texelStepSize = shadowParameters.texelStepSize;    float radius = 1.0;    float dx0 = -texelStepSize.x * radius;    float dy0 = -texelStepSize.y * radius;    float dx1 = texelStepSize.x * radius;    float dy1 = texelStepSize.y * radius;    float visibility = (        czm_shadowDepthCompare(shadowMap, uv, depth) +        czm_shadowDepthCompare(shadowMap, uv + vec2(dx0, dy0), depth) +        czm_shadowDepthCompare(shadowMap, uv + vec2(0.0, dy0), depth) +        czm_shadowDepthCompare(shadowMap, uv + vec2(dx1, dy0), depth) +        czm_shadowDepthCompare(shadowMap, uv + vec2(dx0, 0.0), depth) +        czm_shadowDepthCompare(shadowMap, uv + vec2(dx1, 0.0), depth) +        czm_shadowDepthCompare(shadowMap, uv + vec2(dx0, dy1), depth) +        czm_shadowDepthCompare(shadowMap, uv + vec2(0.0, dy1), depth) +        czm_shadowDepthCompare(shadowMap, uv + vec2(dx1, dy1), depth)    ) * (1.0 / 9.0);
  #else    float visibility = czm_shadowDepthCompare(shadowMap, uv, depth);
  #endif    return czm_private_shadowVisibility(visibility, nDotL, normalShadingSmooth, darkness);
}
#endif
...
vec4 czm_computeAtmosphereColor(
  ...
  float GSq = G * G;
  float rayleighPhase = 3.0 / (50.2654824574f) * (1.0 + cosAngleSq);
  float miePhase = 3.0 / (25.1327412287f) * ((1.0 - GSq) * (cosAngleSq + 1.0)) / (pow(1.0 + GSq - 2.0 * cosAngle * G, 1.5f) * (2.0 + GSq));
...
void czm_computeGroundAtmosphereScattering(vec3 positionWC, vec3 lightDirection, out vec3 rayleighColor, out vec3 mieColor, out float opacity) {
  ...
  czm_computeScattering(primaryRay, length(cameraToPositionWC), lightDirection, atmosphereInnerRadius, rayleighColor, mieColor, opacity);
}
...
vec3 czm_sampleOctahedralProjectionWithFiltering(sampler2D projectedMap, vec2 textureSize, vec3 direction, float lod) {
  ...
  #else        vec3 color = texture(projectedMap, coord).rgb;    
  #endif    return color;
}
vec3 czm_sampleOctahedralProjection(sampler2D projectedMap, vec2 textureSize, vec3 direction, float lod, float maxLod) {

  float currentLod = floor(lod + 0.5f);
...
vec3 czm_sphericalHarmonics(vec3 normal, vec3 coefficients[9]) {
  ...
  float y = normal.y;
  float z = normal.z;
  return L00 + L1_1 * y + L10 * z + L11 * x + L2_2 * (y * x) + L2_1 * (y * z) + L20 * (3.0 * z * z - 1.0) + L21 * (z * x) + L22 * (x * x - y * y);
}
vec3 czm_srgbToLinear(vec3 srgbIn) {
  return pow(srgbIn, vec3(2.2f));
}
vec4 czm_srgbToLinear(vec4 srgbIn) {
  vec3 linearOut = pow(srgbIn.rgb, vec3(2.2f));
...
vec3 getProceduralDiffuseIrradiance(vec3 skyMetrics) {
  vec3 blueSkyDiffuseColor = vec3(0.7f, 0.85f, 0.9f);
  float diffuseIrradianceFromEarth = (1.0 - skyMetrics.x) * (skyMetrics.y * 0.25f + 0.75f) * skyMetrics.z;
  float diffuseIrradianceFromSky = (1.0 - skyMetrics.z) * (1.0 - (skyMetrics.y * 0.25f + 0.25f));
...
vec3 getProceduralSpecularIrradiance(vec3 reflectionWC, vec3 skyMetrics, float roughness) {
  ...
  reflectionWC.x = -reflectionWC.x;
  float inverseRoughness = 1.04f - roughness;
  inverseRoughness *= inverseRoughness;
  vec3 sceneSkyBox = czm_textureCube(czm_environmentMap, reflectionWC).rgb * inverseRoughness;
  vec3 belowHorizonColor = mix(vec3(0.1, 0.15f, 0.25f), vec3(0.4f, 0.7f, 0.9f), skyMetrics.z);
  vec3 nadirColor = belowHorizonColor * 0.5f;
  vec3 aboveHorizonColor = mix(vec3(0.9f, 1.0, 1.2f), belowHorizonColor, roughness * 0.5f);
  vec3 blueSkyColor = mix(vec3(0.18f, 0.26f, 0.48f), aboveHorizonColor, skyMetrics.y * inverseRoughness * 0.5f + 0.75f);
  ...
  float farAboveHorizon = clamp(skyMetrics.x - blendRegionSize * 0.5f + blendRegionOffset, 1.0e-10 - blendRegionSize, 0.99999f);
  float aroundHorizon = clamp(skyMetrics.x + blendRegionSize * 0.5f, 1.0e-10 - blendRegionSize, 0.99999f);
  float farBelowHorizon = clamp(skyMetrics.x + blendRegionSize * 1.5f, 1.0e-10 - blendRegionSize, 0.99999f);
  float notDistantRough = (1.0 - skyMetrics.x * roughness * 0.8f);
...
float getSunLuminance(vec3 positionWC, vec3 normalEC, vec3 lightDirectionEC) {
  ...
  float numerator = ((0.91 + 10.0 * exp(-3.0 * gamma) + 0.45f * NdotL * NdotL) * (1.0 - exp(-0.32f / NdotZenith)));
  float denominator = (0.91 + 10.0 * exp(-3.0 * S) + 0.45f * LdotZenith * LdotZenith) * (1.0 - exp(-0.32f));
...
vec3 textureIBL(vec3 viewDirectionEC, vec3 normalEC, vec3 lightDirectionEC, czm_modelMaterial material) {
  ...
  #ifdef DIFFUSE_IBL        vec3 diffuseContribution = computeDiffuseIBL(cubeDir) * material.diffuse;    
  #else        vec3 diffuseContribution = vec3(0.0);     #endif    float roughness = material.roughness;    
...
vec2 nearestPointOnEllipseFast(vec2 pos, vec2 radii) {
  ...
  vec2 tTrigs = vec2(0.70710678118f);
  vec2 v = radii * tTrigs;
...
void geometryStage(out ProcessedAttributes attributes) {
  ...
  #endif  #ifdef HAS_TANGENTS  
  attributes.tangentEC = normalize(v_tangentEC);  
  #endif  #ifdef HAS_BITANGENTS  
...
float setMetallicRoughness(inout czm_modelMaterial material) {    
  ...
  #endif        vec3 metallicRoughness = texture(u_metallicRoughnessTexture, metallicRoughnessTexCoords).rgb;        
  float metalness = clamp(metallicRoughness.b, 0.0, 1.0);
  float roughness = clamp(metallicRoughness.g, 0.04f, 1.0);        
  ...
  #endif        
  #ifdef HAS_ROUGHNESS_FACTOR            
  float roughness = clamp(u_roughnessFactor, 0.04f, 1.0);        
  #else            float roughness = 1.0;        
  #endif    
  #endif        
  const vec3 REFLECTANCE_DIELECTRIC = vec3(0.04f);
...
void czm_shadow_receive_main() {    
  ...
  ProcessedAttributes attributes;    geometryStage(attributes);    FeatureIds featureIds;    featureIdStage(featureIds, attributes);    Metadata metadata;    MetadataClass metadataClass;    MetadataStatistics metadataStatistics;    metadataStage(metadata, metadataClass, metadataStatistics, attributes);    
...
float fade = max((depth - shadowMapMaximumDistance * 0.8f) / (shadowMapMaximumDistance * 0.2f), 0.0);
