...
vec4 czm_textureCube(samplerCube sampler, vec3 p) {
  #if __VERSION__ == 300    return texture(sampler, p);
  #else    
...
vec3 czm_sphericalHarmonics(vec3 normal, vec3 coefficients[9]){    
  ...
  float y = normal.y;    
  float z = normal.z;    
  vec3 L =          L00        + L1_1 * y        + L10 * z        + L11 * x        + L2_2 * (y * x)        + L2_1 * (y * z)        + L20 * (3.0 * z * z - 1.0)        + L21 * (z * x)        + L22 * (x * x - y * y);            
...
uniform bool u_isInFog;uniform vec4 u_baseColorFactor;
...
struct FeatureIds{    int featureId_0;};
...
void setDynamicVaryings(inout ProcessedAttributes attributes){    
  attributes.featureId_0 = v_featureId_0;}
...
vec3 getNormalEC() {     return normalize(v_normalEC); } 
...
void czm_translucent_main() {     
  ...
  czm_out_FragColor.rgb *= visibility; 
} 
