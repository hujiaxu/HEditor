...
#define HAS_TEXCOORD_0#define SHADOW_MAP
#define OES_texture_float_linear
#define OES_texture_float
#line 0
const float czm_infinity = 5906376272000.0f;
struct czm_ray {
  vec3 origin;
  vec3 direction;
};
...
const czm_raySegment czm_fullRaySegment = czm_raySegment(0.0f, czm_infinity);
...
float czm_approximateTanh(float x) {
  float x2 = x * x;
  return max(-1.0f, min(1.0f, x * (27.0f + x2) / (27.0f + 9.0f * x2)));
}
...
void czm_computeScattering(
  ...
  if(primaryRayAtmosphereIntersect == czm_emptyRaySegment) {
    ...
    rayleighColor = vec3(1.0f, 0.0f, 1.0f);
    return;
  }
  float x = 1e-7f * primaryRayAtmosphereIntersect.stop / length(primaryRayLength);
  float w_stop_gt_lprl = 0.5f * (1.0f + czm_approximateTanh(x));
  start_0 = primaryRayAtmosphereIntersect.start;
  primaryRayAtmosphereIntersect.start = max(primaryRayAtmosphereIntersect.start, 0.0f);
  primaryRayAtmosphereIntersect.stop = min(primaryRayAtmosphereIntersect.stop, length(primaryRayLength));
  float x_o_a = start_0 - ATMOSPHERE_THICKNESS;
  float w_inside_atmosphere = 1.0f - 0.5f * (1.0f + czm_approximateTanh(x_o_a));
  int PRIMARY_STEPS = PRIMARY_STEPS_MAX - int(w_inside_atmosphere * 12.0f);
  int LIGHT_STEPS = LIGHT_STEPS_MAX - int(w_inside_atmosphere * 2.0f);
  float rayPositionLength = primaryRayAtmosphereIntersect.start;
  float totalRayLength = primaryRayAtmosphereIntersect.stop - rayPositionLength;
  float rayStepLengthIncrease = w_inside_atmosphere * ((1.0f - w_stop_gt_lprl) * totalRayLength / (float(PRIMARY_STEPS * (PRIMARY_STEPS + 1)) / 2.0f));
  float rayStepLength = max(1.0f - w_inside_atmosphere, w_stop_gt_lprl) * totalRayLength / max(7.0f * w_inside_atmosphere, float(PRIMARY_STEPS));
  vec3 rayleighAccumulation = vec3(0.0f);
  vec3 mieAccumulation = vec3(0.0f);
  ...
  for(int i = 0; i < PRIMARY_STEPS_MAX; ++i) {
    ...
    float lightPositionLength = 0.0f;
    vec2 lightOpticalDepth = vec2(0.0f);
...
vec3 czm_getDynamicAtmosphereLightDirection(vec3 positionWC, float lightEnum) {
  ...
  const float SCENE_LIGHT = 1.0f;
  const float SUNLIGHT = 2.0f;
...
vec4 geometryStage(inout ProcessedAttributes attributes, mat4 modelView, mat3 normal) {
  ...
  v_positionEC = (modelView * vec4(positionMC, 1.0f)).xyz;    
  #if defined(USE_2D_POSITIONS) || defined(USE_2D_INSTANCING)    
  vec3 position2D = attributes.position2D;
  vec3 positionEC = (u_modelView2D * vec4(position2D, 1.0f)).xyz;
  computedPosition = czm_projection * vec4(positionEC, 1.0f);    
  #else    
  computedPosition = czm_projection * vec4(v_positionEC, 1.0f);    
  #endif        
  ...
  v_positionWC = (czm_model * vec4(positionMC, 1.0f)).xyz;    
  #endif                                                
...
void main() {
  ...
  #ifdef USE_LEGACY_INSTANCING        mat4 instanceModelView;        
  ...
  #endif    #ifdef HAS_POINT_CLOUD_SHOW_STYLE    
