import { Ion, Viewer, CustomShader, VaryingType, TextureUniform, UniformType, LightingModel, ImageryLayer, BingMapsImageryProvider, Model, HeadingPitchRoll, Ellipsoid, Cartesian3, Transforms, IonResource, CameraEventType, PostProcessStage, ScreenSpaceEventHandler, Color, defined, ScreenSpaceEventType, } from "cesium";
  ...
  `,
    ...
    useEffect(() => {
        const cb = async () => {
            ...
            const bingLayer = await BingMapsImageryProvider.fromUrl("https://dev.virtualearth.net", {
            ...
            try {
                ...
                const model = await Model.fromGltfAsync({
                    url: resource,
                    modelMatrix: Transforms.headingPitchRollToFixedFrame(Cartesian3.fromDegrees(0, 0, 100), headingPositionRoll, Ellipsoid.WGS84),
        ...
        `;
                const stage = viewer.scene.postProcessStages.add(new PostProcessStage({
    ...
    return (<div className="w-full h-full relative editor-container">
      <div id="cesiumContainer" className="w-full h-full"></div>
    </div>);
};
export default CesiumBuildingTexture;
//# sourceMappingURL=CesiumGltfTexture.jsx.map